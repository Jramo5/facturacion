...
function downloadStoreConfigJSON() {
    const config = loadStoreConfig();
    const exportData = { factura_store_config: config };
...
function triggerLoadStoreConfig() {
    document.getElementById('load-store-config-file').click();
}

function loadStoreConfigFromFile(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            const config = data.factura_store_config || data;
            if (!config.name) {
                alert('❌ El archivo no tiene el formato correcto.\nDebe contener "factura_store_config" con al menos "name".');
                return;
            }
            saveToLocalStorage('factura_store_config', config);
            applyStoreConfigToInvoice();
            refreshStoreConfigDisplay();
            alert('✅ Configuración de tienda cargada correctamente.');
        } catch(err) {
...
function initializeApp() {
    // v2.6.0: Limpiar datos sensibles de versiones anteriores del localStorage
    _purgeLegacyData();
...
// v2.6.0: Elimina datos sensibles hardcodeados de versiones anteriores
function _purgeLegacyData() {
    const LEGACY_STORE_IDS   = ['store_001', 'store_002'];
    const LEGACY_SELLER_IDS  = ['seller_001', 'seller_002'];
    const LEGACY_CLIENT_IDS  = ['client_001', 'client_002'];
    const LEGACY_PRODUCT_NAMES = [
        'Camisa Manga Larga','Pantalón de Vestir','Vestido Casual',
        'Blusa de Seda','Falda Plisada','Chaqueta Mezclilla',
        'Suéter Tejido','Pantalón Jean'
    ];

    let db = loadFromLocalStorage('factura_database', null);
    if (db) {
        let changed = false;
        ['stores','sellers','clients'].forEach(key => {
            if (!Array.isArray(db[key])) return;
        });
        const storeBefore  = db.stores.length;
        const sellerBefore = db.sellers.length;
        const clientBefore = db.clients.length;

        db.stores  = db.stores.filter(s => !LEGACY_STORE_IDS.includes(s.id));
        db.sellers = db.sellers.filter(s => !LEGACY_SELLER_IDS.includes(s.id));
        db.clients = db.clients.filter(c => !LEGACY_CLIENT_IDS.includes(c.id));

        if (db.stores.length !== storeBefore || db.sellers.length !== sellerBefore || db.clients.length !== clientBefore) {
            saveToLocalStorage('factura_database', db);
            console.log('v2.6.0: Datos sensibles de versión anterior eliminados del localStorage.');
        }
    }

    let products = loadFromLocalStorage('factura_products', null);
    if (products && Array.isArray(products)) {
        const before = products.length;
        products = products.filter(p => !LEGACY_PRODUCT_NAMES.includes(p.name));
        if (products.length !== before) {
            saveToLocalStorage('factura_products', products);
            console.log('v2.6.0: Productos de ejemplo de versión anterior eliminados.');
        }
    }
}
        ...
        <div class="product-edit-form" id="edit-prod-form-${originalIndex}">
            <div class="controls-grid" style="gap:8px;">
                <div class="input-group">
                    ...
                    <div class="product-scan-row">
                        <input id="edit-prod-code-${originalIndex}" value="${p.code || ''}" placeholder="Código de barras">
                        <button class="btn-barcode-inline" onclick="openBarcodeScanner('edit-prod-code-${originalIndex}', null)" title="Escanear">
                            ...
                            <span class="btn-barcode-label">Escanear</span>
                        </button>
                    </div>
                </div>
                <div class="input-group">
...
function loadImage(src) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.crossOrigin = 'anonymous'; // v2.6.1: evita canvas tainted con URLs externas
